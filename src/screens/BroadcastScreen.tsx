...
interface Props {
  ...
  onJoinSquad: (squad: { id: string; name: string; count: number }) => void
}

const MOCK_SQUADS: Squad[] = [
  {
    id: 'mock-1', name: 'Board games @ Union South', count: 6,
    distance: '3 min walk', initials: 'BG',
    avatarBg: '#EEEDFE', avatarColor: '#534AB7',
    blobColor: 'rgba(83,74,183,0.85)', blobSize: 48,
    lat: 43.0741, lng: -89.4077, type: 'join',
  },
  {
    id: 'mock-2', name: 'Photo walk — golden hour', count: 4,
    distance: '7 min walk', initials: 'PW',
    avatarBg: '#FAECE7', avatarColor: '#993C1D',
    blobColor: 'rgba(216,90,48,0.85)', blobSize: 40,
    lat: 43.0788, lng: -89.4095, type: 'join',
  },
  {
    id: 'mock-3', name: 'Calc II study — Library 3rd', count: 3,
    distance: '1 min walk', initials: 'CS',
    avatarBg: '#E1F5EE', avatarColor: '#0F6E56',
    blobColor: 'rgba(29,158,117,0.85)', blobSize: 36,
    lat: 43.0755, lng: -89.4063, type: 'join',
  },
  {
    id: 'mock-4', name: 'Frisbee on the terrace', count: 8,
    distance: '5 min walk', initials: 'FR',
    avatarBg: '#FEF3C7', avatarColor: '#92400E',
    blobColor: 'rgba(180,120,20,0.85)', blobSize: 52,
    lat: 43.0771, lng: -89.4050, type: 'join',
  },
]

export default function BroadcastScreen({ guestId, guestName, myLocation, onLocation, onJoinSquad }: Props) {
  ...
  async function handleJoin(squad: Squad) {
    ...
    onJoinSquad({ id: squad.id, name: squad.name, count: squad.count })
  ...
  async function handlePost() {
    ...
    const loc = myLocation ?? defaultCenter
    const newSquad: Squad = {
      id: crypto.randomUUID(),
      name: newTitle.trim(),
      count: 1,
      distance: 'Just created',
      initials: newTitle.trim().slice(0, 2).toUpperCase(),
      avatarBg: '#EEEDFE', avatarColor: '#534AB7',
      blobColor: 'rgba(83,74,183,0.85)', blobSize: 32,
      lat: loc[0], lng: loc[1], type: 'join',
    }
    setSquads(prev => [newSquad, ...prev])
    setJoined(prev => new Set(prev).add(newSquad.id))
    onJoinSquad({ id: newSquad.id, name: newSquad.name, count: 1 })
    setNewTitle('')
    setCreating(false)
    // Fire-and-forget
    supabase.from('broadcasts').insert({
      user_id: guestId, username: guestName,
      title: newSquad.name, description: '',
